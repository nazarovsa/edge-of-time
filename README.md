# Edge of time

## Materials

### Measure units
- ["Unit of time" - Wikipedia](https://en.wikipedia.org/wiki/Unit_of_time)
- ["Galactic year" - Wikipedia](https://en.wikipedia.org/wiki/Galactic_year)

### Calendars
- ["Julian calendar" - Wikipedia](https://en.wikipedia.org/wiki/Julian_calendar)
- ["Gregorian calendar" - Wikipedia](https://en.wikipedia.org/wiki/Gregorian_calendar)
- ["Islamic calendar" - Wikipedia](https://en.wikipedia.org/wiki/Islamic_calendar)
- ["Japanese calendar" - Wikipedia](https://en.wikipedia.org/wiki/Japanese_calendar)

### Time in Software Engineering
- ["Coordinated Universal Time (UTC)" - Wikipedia](https://en.wikipedia.org/wiki/Coordinated_Universal_Time)
- ["Unix time" - Wikipedia](https://en.wikipedia.org/wiki/Unix_time)
- ["Date in JavaScript"](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date)
- ["DateTimeOffset Struct in c#"](https://learn.microsoft.com/en-us/dotnet/api/system.datetimeoffset?view=net-6.0)
- ["Introduce Date and Time only structs" - issue on dotnet.runtime](https://github.com/dotnet/runtime/issues/49036)
